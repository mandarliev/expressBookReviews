...
regd_users.get("/auth/review/", (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      message: `Hello ${username}, you are authenticated to access this route.`,
    });
  } catch (err) {
    return res.status(401).json({ message: "Unauthorized" });
  }
...
regd_users.delete("/auth/review/:isbn", (req, res) => {
  ...
  if (!book.reviews[username]) {
    res
      .status(404)
      .send(
        `You have not posted any review for the book with ISBN  ${isbn}: ==>${JSON.stringify(
          book
        )}`
      );
    return;
  ...
  res.send(
    `Your review has been deleted for the book with ${isbn} isbn: ==>${JSON.stringify(
      book
    )}`
  );
});
